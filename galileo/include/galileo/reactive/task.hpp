...
namespace galileo
{
    namespace reactive
    {
        ...
        // template <typename Scalar_, int RowsA_, int RowsD_, int NumDecisionVars_>
        // class Task
        // {
        // public:
        //     EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        ...
        //     static constexpr int RowsA = RowsA_;
        //     static constexpr int RowsB = RowsA_;
        //     static constexpr int RowsD = RowsD_;
        //     static constexpr int RowsF = RowsD_;
        //     static constexpr int NumDecisionVars = NumDecisionVars_;

        //     using A_t = Eigen::Matrix<Scalar, RowsA, NumDecisionVars>;
        //     using B_t = Eigen::Matrix<Scalar, RowsB, 1>;
        //     using D_t = Eigen::Matrix<Scalar, RowsD, NumDecisionVars>;
        //     using F_t = Eigen::Matrix<Scalar, RowsF, 1>;
        ...
        //     // Provide an operator* that scales the A, B, D, F matrices by a scalar.
        //     Task<Scalar, RowsA, RowsD, NumDecisionVars> &operator*(const Scalar rhs)
        //     {
        //         a_.noalias() = a_ * rhs;
        //         b_.noalias() = b_ * rhs;
        //         d_.noalias() = d_ * rhs;
        ...
        // template <typename T1, typename T2>
        // static Task<typename T1::Scalar,
        //             math::VConMat<typename std::decay_t<T1>::A_t, typename std::decay_t<T2>::A_t>::type::RowsAtCompileTime,
        //             math::VConMat<typename std::decay_t<T1>::D_t, typename std::decay_t<T2>::D_t>::type::RowsAtCompileTime,
        //             std::decay_t<T1>::NumDecisionVars + std::decay_t<T2>::NumDecisionVars>
        // taskVertcat(T1 &&t1, T2 &&t2)
        // {
        //     using T1dec = std::decay_t<T1>;
        //     using T2dec = std::decay_t<T2>;
        ...
        //     using Scalar = typename T1dec::Scalar;
        //     constexpr int RowsA = math::VConMat<typename T1dec::A_t, typename T2dec::A_t>::type::RowsAtCompileTime;
        //     constexpr int RowsD = math::VConMat<typename T1dec::D_t, typename T2dec::D_t>::type::RowsAtCompileTime;
        //     constexpr int NumDecisionVars = T1dec::NumDecisionVars + T2dec::NumDecisionVars;

        //     AOut a_new = math::vertcat(std::move(t1.a_), std::move(t2.a_));
        //     BOut b_new = math::vertcat(std::move(t1.b_), std::move(t2.b_));
        //     DOut d_new = math::vertcat(std::move(t1.d_), std::move(t2.d_));
        //     FOut f_new = math::vertcat(std::move(t1.f_), std::move(t2.f_));
        //     return Task<T1::Scalar, RowsA, RowsD, NumDecisionVars>(a_new, b_new, d_new, f_new);
        // }
