...
public:
        ...
        : config_(config)
    {
        ...
        if (home_id < 0)
        {
            ...
            init_q_.resize(m_->nq);
            init_q_.setZero();
            default_pose_.resize(m_->nq - 7);
            default_pose_.setZero();
        }
        else
        {
            init_q_.resize(m_->nq);
        ...
        joint_torque_range_.resize(12, 2);
...
private:
                            ...
                            const Eigen::VectorXd &ctrl)
    {
        ...
        idx += 3;
        // ctrl
        obs.segment(nu, idx) = ctrl;
        idx += nu;
        // qpos
        obs.segment(m_->nq, idx) = qpos;
        idx += m_->nq;
        // vb
        ...
        idx += 3;
        // qvel[6..]
        obs.segment(tail_v, idx) = qvel.segment(6, tail_v);
    ...
    double computeReward(StateInfo &info, const Eigen::VectorXd &ctrl)
    {
        ...
        // energy cost
        // Python: sum( max(ctrl[i]*qvel[6+i]/160, 0)^2 )
        double reward_energy = 0.0;
        for (int i = 0; i < joint_range_.rows(); i++)
        {
            int v_idx = 6 + i;
            double val = ctrl[i] * d_->qvel[v_idx] / 160.0;
            if (val < 0.0)
                val = 0.0;
            reward_energy += val * val;
        }
        reward_energy = -reward_energy;

        // sum up with python weights
        double reward = 0.1 * reward_gaits + 0.5 * reward_upright + 0.3 * reward_yaw + 1.0 * reward_vel + 1.0 * reward_ang_vel + 1.0 * reward_height + 0.0 * reward_energy; // 0.00 in snippet
...
int main(int argc, char **argv)
{
    ...
    std::string model_path = "/home/quant/dial_mpc_ws/src/dial-mpc/models/unitree_go2/mjx_go2_force.xml";
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        // e.g. log state.obs, state.reward, etc.
    }
