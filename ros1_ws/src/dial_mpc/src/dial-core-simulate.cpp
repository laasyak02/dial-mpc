...
class ROSNode {
    public:
        ...
        ROSNode(ros::NodeHandle& nh) {
            testPublisher = nh.advertise<std_msgs::Float64MultiArray>("test_topic", 10);
        }
    
        void publishMessage(Eigen::VectorXd value) {
            // std_msgs::Float64 msg;
            // msg.data = value;
            // testPublisher.publish(msg);

            std_msgs::Float64MultiArray msg;
            msg.data.resize(value.size());
...
void ComputeControlTrajectory(const dial::DialConfig &config, go2env::UnitreeGo2Env<NUMSAMPLES + 1> &env, ROSNode& node);
...
static const int NUMBER_OF_SAMPLES = 100; //python-50
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    cfg.traj_diffuse_factor = 0.6; //0.5 //0.7

    go2_config.kp = 30.0;
    go2_config.kd = 0.1; //0.0 //0.1
    go2_config.action_scale = 1.0;
    go2_config.default_vx = 0.0; //1.0
    go2_config.default_vy = 0.0;
    go2_config.default_vyaw = 0.0;
    go2_config.ramp_up_time = 9.0; //2.0 //8.0
    go2_config.gait = "stand"; //trot
    ...
    const std::string model_path = "/root/dial-mpc-python/dial_mpc/models/unitree_go2/mjx_scene_force.xml";
    ...
    ComputeControlTrajectory<NUMBER_OF_SAMPLES>(cfg, env, node);
...
void ComputeControlTrajectory(const dial::DialConfig &config, go2env::UnitreeGo2Env<NUMSAMPLES + 1> &env, ROSNode& node)
{
    ...
    double foot_radius = env.foot_radius_;
    ...
    for (int t = 0; t < config.n_steps; t++)
    {
        ...
        go2env::Vector4d z_feet_val= next_state.info.z_feet;
        // std::cout << "z_feet: " << z_feet_val << std::endl;
        go2env::Vector4d foot_contact_z = z_feet_val.array() - foot_radius;
        Eigen::Array<bool, 4, 1> contact = (foot_contact_z.array() < 1e-3);
        // std::cout << "contact: " << contact << std::endl;

        // std::vector<std::array<double, 3>> contactForces = next_state.info.contactForces;
        // for (int i = 0; i < contactForces.size(); i++) {
        //     std::cout << "Force: [" << i << ": " << contactForces[i][0] << ", " << contactForces[i][1] << ", " << contactForces[i][2] << "]" << std::endl;
        // }

        std::map<std::string, std::array<double, 3>> footContacts = next_state.info.footContacts;
        std::vector<double> concatenatedForces;

        for (const auto& [footName, Force] : footContacts) {
            concatenatedForces.insert(concatenatedForces.end(), Force.begin(), Force.end());
        }
        // std::cout << "Forces size" << concatenatedForces.size() << std::endl;

        // for (const auto& [footName, Force] : footContacts) {
        //     std::cout << ", Foot: " << footName 
        //               << ", Force: [" << Force[0] << ", " 
        //               << Force[1] << ", " << Force[2] << "]" 
        //               << std::endl;
        // }

        std::vector<double> jointAccelerations = next_state.info.jointAccelerations;
        // std::cout << "Acc_j size" << jointAccelerations.size() << std::endl;
        // std::cout << "Joint Accelerations: [";
        for (size_t i = 0; i < jointAccelerations.size(); i++) {
            // std::cout << jointAccelerations[i] << ",";
        }
        // std::cout << "\n";

        Eigen::Map<Eigen::VectorXd> accMap(jointAccelerations.data(), jointAccelerations.size());
        Eigen::Map<Eigen::VectorXd> forceMap(concatenatedForces.data(), concatenatedForces.size());

        Eigen::VectorXd finalVector_u(accMap.size() + forceMap.size() + action.size());
        finalVector_u << accMap, forceMap, action;
        // std::cout << "finalVector_u size" << finalVector_u.size() << std::endl;

        // node.publishMessage(t*1.0);
        node.publishMessage(finalVector_u);
        std::cout << "Published message, t: " << t << "\n";
        // std::cin.get();
