...
namespace go2env
{
    ...
    public:
        ...
        //     pipeline_state.x.pos_ << -3.8306182e-04, -1.0715606e-07, 2.6867321e-01;
            ...
            // Making jointAccelerations of size 18 with com linear + angular acc + joint acc
            std::vector<double> jointAccelerations(18);
            // Copy the accelerations from mjData
            for (int i = 0; i < 18; i++) {
                jointAccelerations[i] = d->qacc[i];
            }
            new_state.info.jointAccelerations = jointAccelerations;
        ...
        EnvState reset(std::mt19937_64 &rng)
        {
            ...
            // Making jointAccelerations of size 18 with com linear + angular acc + joint acc
            info.jointAccelerations = std::vector<double>(18, 0.0);
