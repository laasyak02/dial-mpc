...
namespace galileo
{
    namespace reactive
    {
        ...
        public:
            ...
            vector_t update(const Eigen::MatrixBase<DesStateVector> &stateDesired, const Eigen::MatrixBase<DesControlVector> &controlDesired, const Eigen::MatrixBase<RBDStateVector> &rbdStateMeasured, size_t mode, NumScalar period, std::vector<double>& u_guess)
            {
                ...
                //Initial guesses
                // vector_t x0(getNumDecisionVars());
                // vector_t x0 = const Eigen::Map<vector_t>(u_guess.data(), u_guess.size());
                Eigen::Map<vector_t> x0(u_guess.data(), u_guess.size());


                // qpProblem.init(H.data(), g.data(), A.data(), nullptr, nullptr, lbA.data(), ubA.data(), nWsr);
                qpProblem.init(H.data(), g.data(), A.data(), nullptr, nullptr, lbA.data(), ubA.data(), nWsr, x0.data()); // with initial guesses
